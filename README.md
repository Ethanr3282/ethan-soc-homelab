...
Three virtual machines on a host-only network running in UTM on a MacBook Air (32 GB RAM, Apple Silicon):

- **Ubuntu Server ARM64** (192.168.64.10) — Wazuh SIEM (manager + indexer + dashboard)
- **Windows 11 Enterprise ARM** (192.168.64.20) — victim endpoint with Sysmon + Wazuh agent
- **Kali Linux ARM64** (192.168.64.30) — attacker box for simulated attacks
...
UTM (QEMU) · Ubuntu Server 24.04 ARM64 · Windows 11 ARM · Kali Linux ARM64 · Wazuh · Sysmon · Atomic Red Team · MITRE ATT&CK · Python · Anthropic API
